...
- [ ] `src/alpha_agent/gateway/session.py`：
  - 删除 `MemoryScope` 构造；`gateway_session_mappings.memory_scope` 列在
    Phase 00 暂保留为空 JSON（schema 不动），Phase 02 之后改由
    CounterpartRouter / ThreadId 接管来源语义。
  - `GatewaySessionMapping.memory_scope` 重命名为 `source_context` 或收缩为
    `metadata` 中的 source 字段，具体选择写入本阶段决策表。
- [ ] `src/alpha_agent/gateway/runner.py`：
  - 不再把 `memory_scope` 写入 runtime `source_metadata`。
  - 保留 platform / chat / user / thread 等 source 元数据，供 Phase 02 的
    CounterpartRouter 使用。
...
- [ ] `config.example.toml` 与 `DEFAULT_CONFIG_TOML` 同步删除上述键。
- [ ] `alpha config show`、`alpha config set/get` 的支持键与输出同步收缩。
...
- [ ] 更新仍保留的测试中所有 memory/config 依赖：
  ...
  - `tests/test_cli_agent_loop.py` 只保留 ask/chat/debug/skills/gateway 的行为
    断言；memory 子命令相关用例删除或迁到后续 cognition CLI 测试。
...
- [ ] `docs/todo/TODO.md`：把"keep memory native to Alpha Agent: working,
...
src/alpha_agent/gateway/session.py      （删除 MemoryScope 语义）
src/alpha_agent/gateway/runner.py       （source_metadata 不再携带 memory_scope）
config.example.toml
...
- [ ] `grep -rn "memory\.retrieval_limit\|ALPHA_MEMORY\|ALPHA_RETRIEVAL_LIMIT\|
...
- [ ] `alpha memory ...` 子命令全部不存在；`alpha --help` 输出里没有
  `memory` 字样。
- [ ] `alpha config show` 不再输出 memory/capture/retrieval/consolidation
  配置项。
