...
- [ ] `cognition/reflectors/__init__.py`。
- [ ] `cognition/reflectors/l1.py`：`ReflectorL1` 类。
  - 构造接收 ProjectionRegistry（读 BeliefProjection 等）。
  - `audit(perception, focus, interpretation, judgments, decision, outcome,
    feedback) -> list[Reflection]`。
  - 内部把规则注册成 plug-in callable，便于增删。
- [ ] `cognition/reflectors/rules/`：每条规则一个文件，统一签名（见 §3.1）。
...
- [ ] `cognition/projections/reflection.py`：`ReflectionProjection`。
- [ ] `state/schema.sql` 追加 `reflection_view` 表。

### 2.4 Reactive 接入

- [ ] `cognition/controller.py`：把 Phase 02 留下的空 reflector 调用替换。
- [ ] `cognition/stages/reflect.py`：调 `ReflectorL1.audit(...)`，emit
  `reflected` 事件（含 reflection_count）+ 每条 Reflection 一个独立事件。

### 2.5 CLI

- [ ] `cli.py`：新增 `alpha cognition reflections` 子命令。
...
- [ ] 更新 AGENTS.md。
- [ ] 在 cognition/reflectors/ README 简列 v1 规则及触发条件。
...
| name                          | 触发                                          | severity |
| ----------------------------- | -------------------------------------------- | -------- |
| low-confidence-high-stakes    | judgment.confidence<0.4 且 existence 权重>0.7 | warning  |
...
| premature-novel-auto-form     | stance=novel 且 confidence<0.5 但被 belief_formed | warning  |
...
{"tick_id": ..., "reflection_count": N, "reflection_ids": [...]}
...
- [ ] `uv run pytest tests/cognition/test_rule_*.py -q` 全绿。
- [ ] `alpha cognition reflections --last 10` 能列出最近反思。
- [ ] 连续输入两条矛盾偏好 → 第二条触发 `contradiction-accepted` reflection。
- [ ] 任何 tick 跑完，event log 里都有 `reflected` 事件（即使 reflection_count=0）。
- [ ] drop reflection_view → replay → 等价。
