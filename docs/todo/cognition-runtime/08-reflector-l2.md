...
L2 是 scheduler-compatible worker，可通过 Phase 06 Scheduler 以
priority=L2 跑。Phase 08 v1 不引入后台 daemon scheduler。
...
v1 支持 global override 和 Counterpart-scoped override 的匹配 helper；
Drive Loop self_signal 匹配等 Drive Loop 相关路径仍 deferred。
...
- [x] `cognition/reflectors/l2.py`：`ReflectorL2`。
  - `run()`：扫近期 reflection + events → emit 若干 `strategy_changed`。
  - 由共享 Scheduler 负责 acquire；worker 每条规则评估完后调 yield。
...
- [x] `cognition/reflectors/l2_rules/`：每条 L2 规则一个文件。
...
| feedback-surprise-streak          | 同 trigger 下 feedback 不匹配预期连续 ≥5 次；同 trigger 成功会重置 streak | `disable_auto_procedure_match_for_trigger` |
| lens-shift-flap                    | 同 lens shift 方向反复 ≥3 次（24h 内）；方向从 before/after priority 或 sensitivity delta 派生 | `freeze_lens_learning_for_24h`       |
| premature-novel-auto-form-burst   | 1 小时内 novel 自动 belief_formed ≥5 次       | `require_confirm_before_novel_form`  |

### 2.3 StrategyProjection

- [x] `cognition/projections/strategy.py`：`StrategyProjection`。
  - handles `strategy_changed`、`strategy_expired`。
  - `active(subject) -> list[StrategyOverride]`：当前所有有效 override。
  - `is_active(subject, name) -> bool`。
- [x] `strategy_view` 表（见 §3.2）。

### 2.4 Reactive 套用 strategy

- [x] `cognition/controller.py`：每个 tick 从 StrategyProjection 取
  当前 override，传给 stage。
- [x] 改 stage：
  - `interpret.py`：若 `require_confirm_before_novel_form` 活跃 → 标记
    Interpretation 含 `requires_confirmation=True`。
  - `controller.py`：若 `disable_auto_procedure_match_for_trigger` 活跃 →
    在进入 Decide 前跳过 Procedure 匹配。
  - `revise.py`：若 `require_explicit_confirm_on_contradiction` 活跃 →
    contradicting 时 emit `belief_form_pending_confirmation`。v1 不构造完整
    pending belief candidate，因为 Reactive Reviser 尚未自动 form belief。
  - `loops/workers/learn_value_lens.py`：若 `freeze_lens_learning_for_24h`
    活跃 → 跳过。

### 2.5 过期机制

- [x] `cognition/loops/workers/expire_strategies.py`：扫 active strategy 检查
...
- [x] `alpha cognition strategies [--active] [--all]`。
- [x] `alpha cognition strategy-expire <id>` 手动撤销。

### 2.7 测试

- [x] `tests/cognition/test_reflector_l2_phase08.py` 覆盖 L2 规则 emit、
  StrategyProjection/expiry、CLI、interpret/revise/lens learning/decide 应用。
- [x] Counterpart matching：
  ...
  - Drive Loop self_signal 仍 deferred，等待 Phase 10 Drive Loop 存在。

### 2.8 文档

- [x] AGENTS.md。
- [x] `cognition/reflectors/l2_rules/README.md` 列规则与对应 strategy。
...
docs/develop_record/phase-08-reflector-l2.md
```
...
src/alpha_agent/cognition/loops/workers/__init__.py         注册 expire_strategies
src/alpha_agent/cognition/models/_ids.py                    StrategyId
src/alpha_agent/cognition/models/__init__.py                StrategyOverride export
src/alpha_agent/cognition/models/event.py                   Phase 06/07 已含所需事件 kind
...
- [x] `uv run pytest tests/cognition/test_reflector_l2_phase08.py -q` 全绿。
- [x] 演示：连续 3 次矛盾 reflection → L2 emit `strategy_changed` →
  第 4 次矛盾输入时 Reactive 走"需要显式确认"分支。
- [x] strategy valid_until 到 → 自动 emit `strategy_expired` → 下一轮恢复
  正常行为。
- [x] `alpha cognition strategies --active` 列出当前活跃 strategies。
- [x] L2 不会 emit 任何 `belief_*` 事件（只发 strategy_*）。

## 6. 风险与备注

- **strategy 名字稳定性**。每个 name 是 stage 代码识别的 enum；命名一旦发布
  不能改，否则历史事件 replay 行为异变。如要改，加新 name 并 deprecate 旧的。
- **过度策略化风险**。L2 emit 太多 strategy 会让 Reactive 行为反复变。
  `StrategyProjection` 在 materialization 边界设硬上限：同主体同时 active
  non-expired strategies ≤ 5；超过上限的新 `strategy_changed` 被确定性跳过。
