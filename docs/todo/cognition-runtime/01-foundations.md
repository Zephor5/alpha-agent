...
- Phase 01 已落地为 `src/alpha_agent/cognition/` 模块：models / event_log /
  projections / coordinator 四条基础线均已实现并验证。
- 标识与轻量值采用 `NewType` typed alias；跨对象引用与可序列化结构采用
  frozen dataclass skeleton。复杂认知行为仍留给后续阶段。
- SQLite event log 复用 `StateStore` 的连接与事务，不在 cognition 层自行打开
  数据库；`cognitive_events` 是 append-only 写入通道。
- `counterpart_view` 已作为 `CounterpartProjection` 的物化视图落地；
  service committed / fulfilled / failed 等 service events 以 JSON 形式追加到
  `service_contract`。
- `LoopCoordinator` 是线程级、单 Subject、协作式调度实现；不会抢占或取消
  holder，只通过 `yield_to_higher_priority()` 主动让位。
- Reactive loop 尚未接入，本阶段没有改变 `AlphaAgent.respond()` 的行为。
...
- [x] `cognition/models/_ids.py`：新类型别名集合
...
- [x] `cognition/models/subject.py`：`Subject`——单例，含
  ...
  不允许出现字面量 `"agent:self"`。
- [x] `cognition/models/counterpart.py`：`Counterpart` + `ServiceCommitment` +
  `Relationship` + `StyleHint`。
- [x] `cognition/models/belief.py`：`Belief`，含 `about: list[Reference]`
  表达"这条信念关于谁"。
- [x] `cognition/models/situation.py`：`Situation`；其 SocialContext 引用
  当前在场的 Counterpart 列表。
- [x] `cognition/models/perception.py`：`Perception`，含
  `from_counterpart: CounterpartRef | None`；`Stimulus`，含
  `source: CounterpartRef | None`。
- [x] `cognition/models/judgment.py`：`Judgment`。
- [x] `cognition/models/decision.py`：`Decision`。
- [x] `cognition/models/reflection.py`：`Reflection`。
- [x] `cognition/models/procedure.py`：`Procedure`。
- [x] `cognition/models/context_window.py`：`ContextWindow`，含
  `counterpart: CounterpartRef | None`——会话 thread 关联到一个 Counterpart；
  cognition thread 为 None。一并定义 `CompressedSummary`。
- [x] `cognition/models/thread.py`：`ThreadId` 数据类与 `ThreadKind` 枚举
...
- [x] `cognition/models/value.py`：`ValueProfile`、`ValueLens`。
- [x] `cognition/models/event.py`：`CognitiveEvent`。
- [x] `cognition/models/__init__.py`：聚合 export。

所有 dataclass 一律 `frozen=True`。所有列表/字典字段都用 `field(default_factory=...)`。

### 2.2 EventLog 与 EventEmitter

- [x] `cognition/event_log/base.py`：`EventLog` Protocol（见 §3.2）。
- [x] `cognition/event_log/memory.py`：`InMemoryEventLog`。
- [x] `cognition/event_log/sqlite.py`：`SQLiteEventLog`。
- [x] `cognition/emitter.py`：`EventEmitter`——`EventLog.append` 的轻量
...
- [x] `state/schema.sql`：在 Phase 00 留下的 schema.sql 末尾追加
  `cognitive_events` 表（见 §3.3）。注意 schema 文件归属——事件日志虽然属于
  cognition，但表与现有 `conversation_messages` 共库，所以 DDL 仍放在
  `state/schema.sql`。
- [x] `state/store.py`：扩展 `StateStore` 以提供事件日志相关连接（不暴露事件
  逻辑，只提供 sqlite connection）。
- [x] `cognition/event_log/sqlite.py` 通过 `StateStore` 取连接，不自己 open。

### 2.3 Projection

- [x] `cognition/projections/base.py`：
  - `Projection` 抽象基类，方法：`apply(event) -> None`、`reset() -> None`、
    `view() -> ProjectionView`。
  - `ProjectionView` 标记接口。
- [x] `cognition/projections/registry.py`：`ProjectionRegistry`——按名字
...
- [x] `cognition/projections/event_count.py`：demo `EventCountByKind`
  projection。
- [x] `cognition/projections/counterpart.py`：`CounterpartProjection`——
...
- [x] `cognition/projection_runner.py`：通用 replay runner——给定一个
  EventLog 与一组 Projection，按事件流顺序 dispatch。

### 2.4 调度：LoopCoordinator

- [x] `cognition/coordinator.py`：`LoopCoordinator`（见 §3.6）。
...
- [x] `cognition/coordinator.py` 同文件：`LoopPriority` 枚举（reactive=0、
...
- [x] `tests/cognition/test_event_log_memory.py`：append/iter/length。
- [x] `tests/cognition/test_event_log_sqlite.py`：append、replay 不丢序、跨
  进程持久化。
- [x] `tests/cognition/test_projection_runner.py`：用 EventCountByKind 验证
  - 同一事件流跑两次得到等价 view；
  - 中途 reset 后能从头重建；
  - 新增 projection 后能从历史回放出当前状态。
- [x] `tests/cognition/test_types_frozen.py`：所有模型类型都是 frozen 且可
  hash（如果需要）。
- [x] `tests/cognition/test_counterpart_projection.py`：
  - first_observed / identified / relationship_changed / trust_updated 事件
    序列 → view 状态正确；drop view → replay → 等价。
- [x] `tests/cognition/test_belief_about_field.py`：构造 Belief 含 `about=
  [counterpart_ref, entity_ref]`，序列化/反序列化往返一致。
- [x] `tests/cognition/test_loop_coordinator_serial.py`：
  - 两个调度型 acquire 串行执行；高优先级 acquire 在低优先级 holder 释放
    前阻塞。
- [x] `tests/cognition/test_loop_coordinator_try_acquire_busy.py`：
  - 低优先级 holder 持锁中 → Reactive try_acquire 立即抛 LockBusy，
    LockBusy.holder 与 since 字段正确填充。
- [x] `tests/cognition/test_loop_coordinator_try_acquire_free.py`：
  - 无 holder → Reactive try_acquire 立即拿到锁，正常退出。
- [x] `tests/cognition/test_loop_coordinator_yield.py`：
  - 低优先级 holder 在 `max_chunk_duration` 后调 yield → 若有更高调度型
    优先级等待则让位，否则 holder 续锁。
- [x] `tests/cognition/test_loop_coordinator_no_preemption.py`：
  - 即使 Reactive 反复 try_acquire 失败，coordinator 不会强制让低优先级
    holder 释放——只能由 holder 主动 yield。

### 2.6 文档

- [x] 更新 `AGENTS.md` 项目导航：`src/alpha_agent/cognition/` 一行 + 简介。
- [x] 在仓库根 `README.md` 加一行 "Cognition runtime under construction; see
...
`schema_version` 为后续事件 schema 演化预留；本项目不以长期兼容旧 schema 为
目标，后续阶段如需调整事件结构，应同步重构 replay / projection。
...
- [x] 模块 import 检查：`python -c "from alpha_agent.cognition import models,
  event_log, projections, coordinator"` 不报错。
- [x] SQLite event log：append 1000 条 → 关进程 → 重开 → iter 出来 1000 条且
  顺序一致。
- [x] EventCountByKind projection：清空状态后 `replay_all`，view 与逐事件
  apply 等价。
- [x] CounterpartProjection：drop `counterpart_view` → replay → 等价。
- [x] 所有模型类型 `dataclasses.is_dataclass(x)` 与 `getattr(x, "__hash__",
  None) is not None`（frozen=True 给定的 hash）通过。
- [x] `Belief.about` 字段在序列化/反序列化中保持 list 顺序与引用类型。
- [x] LoopCoordinator 单元测试覆盖：
  - 串行 acquire / release；
  - 高优先级阻塞等待，holder 主动 yield 时让位；
  - 无 yield 时不被强制中断（即 acquire 调用时不会触发任何信号 / 异步取消）。
- [x] `AlphaAgent.respond()` 行为与 Phase 00 末态**完全一致**——这一阶段
  不接触它。

## 6. 风险与备注

- **类型一次写齐 vs 渐进**。实际实现采用 typed alias + frozen dataclass
  skeleton：标识和值对象先保持轻量，跨对象引用和可序列化实体保留类型边界。
  这让后续阶段能直接在统一模型上补行为，而不再恢复旧记录体系。
- **schema 演化策略**。本项目不做兼容优先的渐进替换；如果 Phase 02+ 发现
  事件字段或投影视图方向错误，应按目标架构直接重构。`schema_version` 只用于
  标记事件格式，不代表要长期保留旧 schema。
- **payload 逃生通道**。某些字段还没想清楚时（例如 `derivation` 怎么序列化），
  先放 payload。稳定后直接 promote 到正式字段，并同步更新 replay / projection。
- **SQLite 边界**。`SQLiteEventLog` 通过 `StateStore` 获取连接和事务，共用
  state schema；cognition 模块不单独 open 数据库，避免状态所有权分叉。
- **service events 物化**。`CounterpartProjection` 已把 committed / fulfilled /
  failed 事件作为 JSON service record 追加到 `service_contract`，保持事件流为
  源头、`counterpart_view` 只做查询视图。
