...
projection 仍是 stub / replay-backed 简化实现，Phase 03 / 04 才会用真实
BeliefProjection 与 ContextWindowProjection 替换。

## Completion notes

Phase 02 已接入 `AlphaAgent.respond()`：成功路径通过
`CognitiveController.reactive_tick(...)` 跑完 Reactive 单 tick，busy 路径
保持非阻塞且不写 cognitive event / conversation message。实际实现按阶段边
界保留 9 个 stage 文件，但本阶段刻意做了以下简化：

- Projection 仍是 stub / replay-backed：Belief 与 Procedure 返回空，
  ContextWindow 只取最近 perceived foreground，Subject 从事件流重建或返回
  `SUBJECT_SELF` 默认值。
- 默认 `Effector` 自己执行 bounded tool loop：一轮工具迭代加 final LLM
  round，满足 `CognitiveController` 默认路径。`AlphaAgent.respond()` 在
  Reactive Effector 边界注入 runtime runner，用于持久化 transcript 与 tool
  traces；Renderer 未拆出，Phase 09 再把 prompt/render 责任从 Effector 中分
  离。
- Reflect 与 Reviser 只发 placeholder 事件，不形成真实 Reflection 或
  Revision；Phase 05 以后再补规则与衍生事件。
- Attend 与 Interpret 未合并，继续作为独立 stage 保留，便于 Phase 03 接入
  real belief recall 后扩展解释语义。
...
- `Effector` 调 LLM / tool——这是这一阶段唯一调外部世界的地方。默认实现内部
  执行 bounded tool loop（一轮工具迭代 + final LLM round），LLM call 走现有
  `alpha_agent/llm/` 抽象；`AlphaAgent` 注入 runtime runner 负责把最终
  transcript 与 tool traces 落到 runtime state。
...
- ValueLens 冲突解决（Phase 07；Judger 这一阶段直接采用所有 Judgment，不解
  冲突）。
- Renderer 解耦（Phase 09；这一阶段 Effector 在边界用 `decision/window` 直接
  拼最简 prompt）。
...
- [x] `cognition/stages/perceive.py`：`Perceiver.perceive(stimulus, subject,
...
- [x] `cognition/stages/attend.py`：`Attender.focus(perception, subject) ->
  AttentionFocus`，emit `attended`。
- [x] `cognition/stages/interpret.py`：`Interpreter.interpret(focus, window,
  recalled, subject) -> Interpretation`，emit `interpreted`。
- [x] `cognition/stages/judge.py`：`Judger.judge(interpretation, value_lens)
  -> list[Judgment]`，emit `judged`。
- [x] `cognition/stages/decide.py`：`Decider.decide(judgments, procedures,
  subject, window) -> Decision`，emit `decided`。
- [x] `cognition/stages/effector.py`：`Effector.execute(decision, window) ->
  Outcome`，emit `acted`。LLM call 落在这里。
- [x] `cognition/stages/feedback.py`：`FeedbackReader.compare(decision,
  outcome) -> Feedback`，emit `received_feedback`。
- [x] `cognition/stages/reflect.py`：`ReflectorL1.audit(...) ->
  list[Reflection]`，emit `reflected`。这一阶段返回空列表，Phase 05 加规则。
- [x] `cognition/stages/revise.py`：`Reviser.derive(...) -> list[Revision]`，
  emit `revised` placeholder。这一阶段不形成 `belief_formed` /
  `belief_superseded` 等下游事件，Phase 03+ 有真实 belief 后再接入。
...
- [x] `cognition/controller.py`：`CognitiveController.reactive_tick(...)`。
  - 内部按顺序调 stage。
...
- [x] `cognition/projections/subject.py`：`SubjectProjection`。先用 replay
  事件流的方式重建 Subject 字段；找不到就返回 `Subject.default()`（id 固
  定为 `SUBJECT_SELF`）。
- [x] `cognition/projections/belief.py`：`BeliefProjection` stub——`recall`
  永远返回空列表；`status` 字段 `"stub"`，便于 inspection 区分。Phase 03 替
  换实现。
- [x] `cognition/projections/procedure.py`：`ProcedureProjection` stub——
  `match` 永远返回空。
- [x] `cognition/projections/context_window.py`：`ContextWindowProjection`
...
- [x] `runtime/agent.py`：`AlphaAgent.respond` 改为：
...
- `_compose_busy_message(busy)`：返回一条对外可读的系统提示，例如
  `"Agent is currently consolidating cognition (started 12s ago); please retry
...
- [x] `runtime/counterpart_router.py`（新）：`CounterpartRouter`，封装
...
- [x] 删除 Phase 00 留下的最简 prompt 拼接逻辑——`AlphaAgent.respond()`
  不再预先用 `SessionContextManager` + `PromptBuilder` 构造 LLM 输入；prompt
  在 Reactive Effector 边界由 `decision/window` 构造。
- [x] `runtime/prompt_builder.py`：标记为 deprecated；Phase 09 整合到
  Renderer。Effector 内部暂时从 `decision/window` 直接拼 chat messages。

### 2.5 测试

- [x] `tests/cognition/test_reactive_tick_happy_path.py`：
  - 给一条用户 stimulus → tick 跑完 → event log 里有 9 类事件且因果链正确。
  - LLM 用 mock provider。
- [x] `tests/cognition/test_reactive_tick_tool_call.py`：
  - Decision = use_tool → Effector 触发 tool registry → tool result 作为
    feedback。
- [x] `tests/cognition/test_controller_emits_tick_id.py`：
  - 一轮 tick 内所有事件 payload 含同一个 `tick_id`。
- [x] `tests/cognition/test_stub_projections.py`：
  - ContextWindow stub 能取出最近 K 个 perceived。
  - Belief/Procedure stub 返回空。
- [x] `tests/cognition/test_counterpart_router.py`：
...
- [x] `tests/cognition/test_reactive_busy_response.py`：
...
- [x] `tests/cognition/test_reactive_busy_no_counterpart_observation.py`：
  - 模拟第一次见的新 (platform, user_id) 在 busy 状态下被拒 →
    `counterpart_first_observed` 事件**不**被 emit；下次锁空闲时第一条消
    息才触发 first_observed。
- [x] `tests/cognition/test_reactive_no_preemption.py`：
...
- [x] AGENTS.md 项目导航更新：`cognition/stages/`、`cognition/controller.py`、
  `cognition/projections/`。
- [x] 在 `docs/develop_record/` 留一篇 dev note，记录本阶段对 stage 边界的
...
- [x] 本轮 9 类事件及其因果父事件可通过 cognitive event log 验证。
- [x] 同一 session 中连续 5 轮对话，每轮 event log 多 9 条，无丢失。
- [x] 工具调用路径走通：用户发"用 X 工具做 Y"，能看到 `decided`(use_tool) →
  `acted` → tool result → `received_feedback`。
- [x] Phase 00 留下的 xfail 测试中，"上一轮提到的事下一轮能回忆"类的用例**仍
  然 xfail**——这要等 Phase 03 BeliefProjection 真实化才能转绿。
- [x] `AlphaAgent.respond()` 对外签名与 Phase 00 末态完全一致。
- [x] Event log 中每条事件的 `causal_parents` 至少有一项（除 `perceived` 之
  外）。
...
- **Effector 的 LLM/tool call 是这一阶段最大的复杂度**。默认 Effector 自己
  执行 bounded tool loop（一轮工具迭代 + final LLM round），并通过
  `Effector.execute(decision, window)` 收口。`AlphaAgent` 注入的 runtime
  runner 不负责预构造 prompt，只负责在 Effector 边界执行并持久化
  transcript / tool traces。
