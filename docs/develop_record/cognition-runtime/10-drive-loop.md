...
**Status:** completed (deterministic synchronous v1)
...
- `GoalRegistry`：set_goal / satisfy / abandon / progress。
- `DriveLoop` 同步 v1：暴露 scheduler trigger 形状，但本阶段 CLI/测试直接调用
  `run_once(force=True)`；没有后台 daemon scheduler。
...
- [x] `cognition/models/goal.py`：`Goal` frozen dataclass（id / description /
...
- [x] 把 `Goal` / `GoalId` 加到 `cognition/models/__init__.py` export。
- [x] `CognitiveEventKind` 增加 `goal_set` / `goal_satisfied` /
  `goal_abandoned` / `goal_progressed`。

### 2.2 GoalRegistry

- [x] `cognition/goals/registry.py`：
...
- [x] `cognition/projections/goal.py`：`GoalProjection`，含 active cap 与 event-log
  rebuild。
- [x] `state/schema.sql` 追加 `goal_view` 表（见 §3.1），并额外保存
  `last_event_id` 支持投影可追踪。

### 2.3 DriveLoop

- [x] `cognition/loops/drive.py`：`DriveLoop`。
  ...
  - Stimulus.source = goal.for_counterpart（若有），让 Reactive 知道这次思考
    "是关于 user_a 的"。
  - 暴露 Phase 06 scheduler trigger 形状，触发条件：
    ...
    v1 没有后台 daemon scheduler；`alpha cognition drive --once` 直接手动触发。
- [x] cooldown 配置：每 goal 至少间隔 T 秒才能再生成 self-stimulus（goal 维
  度节流，独立于 scheduler 全局节流）。

### 2.4 self-stimulus 路由

- [x] `cognition/stages/perceive.py`：已有 `Stimulus.kind="self_signal"` →
  Perception.source = `self_signal`，situation.historical 与 conversation
  thread 隔离。
- [x] ThreadId.cognition 已经在 Phase 04 准备好；本阶段保证 Drive Loop 用对。

### 2.5 配置 & CLI

- [x] `config.cognition.drive.enabled = false`（默认关），与 Phase 06 的
  `config.cognition.consolidation.*` 使用同一 `cognition` 配置命名空间。
- [x] `alpha cognition goals list [--active] [--subject]`。
- [x] `alpha cognition goals set --description ... [--priority N]`。
- [x] `alpha cognition goals satisfy <id> --evidence ...`。
- [x] `alpha cognition goals abandon <id> --reason ...`。
- [x] `alpha cognition drive --once` 手动跑一次（即使 enabled=false）。

### 2.6 测试

- [x] `tests/cognition/test_goal_registry_emits.py`。
- [x] `tests/cognition/test_goal_projection_rebuild.py`。
- [x] `tests/cognition/test_drive_loop_behavior.py`：set goal →
  drive run_once → 看到 perceived (self_signal) 与后续完整 reactive 链。
- [x] `tests/cognition/test_drive_loop_behavior.py`：cooldown 内不重复触发。
- [x] `tests/cognition/test_drive_loop_behavior.py`：disabled 默认不触发，force 可跑。
- [x] `tests/cognition/test_cli_goals.py`。

### 2.7 文档

- [x] README.md。
- [x] AGENTS.md。
- [x] `cognition/loops/README.md` 加 Drive Loop 说明。
...
src/alpha_agent/cognition/models/_ids.py        GoalId
src/alpha_agent/cognition/models/enums.py       goal_* kind 已接入事件枚举
src/alpha_agent/cognition/controller.py         default_projection_registry 注册 GoalProjection
...
- [x] `uv run pytest tests/cognition/test_goal_*.py
  tests/cognition/test_drive_loop_*.py tests/cognition/test_cli_goals.py -q` 全绿。
- [x] 设一个 goal "answer pending question X" → `alpha cognition drive --once`
  → event log 出现完整 self_signal Reactive 链。
- [x] cooldown 内重复 run_once 不再触发同 goal。
- [x] 默认配置下 enabled=false，Drive Loop 不自动跑（仅手动 `--once` 可用）。
- [x] satisfy/abandon 后该 goal 不再被选。
- [x] self_signal 走 cognition thread，conversation thread 的 ContextWindow
  不受污染。

## 6. 风险与备注

- **自主行为的安全边界**。Drive Loop 一旦上线会让主体"自己说话"。v1 默认关闭、
