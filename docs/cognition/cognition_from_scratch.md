...
之前的 `cognition_implementation.md` 妥协地"在 memory 之上加一层薄认知运行
时"——那是工程上正确的渐进路径，但它**把 memory 当主语**，认知是寄生层。

这一份文档反过来。**主语是认知，存储是从认知派生的副产物**。如果完全不考虑现
有 memory 实现、从 cognition.md 的需求一步步推导，我会得到一个**以信念为中心、
以事件日志为基底、以多并发 loop 为运行机制、以元认知为内建机制**的系统。下文
把这个系统写完整。
